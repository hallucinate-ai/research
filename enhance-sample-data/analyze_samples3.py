...
print(np.corrcoef(diff_similarity, cfg_scale))
print("dst_aesthetic_score vs ddim_steps")
print(np.corrcoef(diff_similarity, ddim_steps))
print("dst_aesthetic_score vs denoising_strength")
print(np.corrcoef(diff_similarity, denoising_strength))
plt.show()
...
length = int(round(len(diff_similarity) / 2))
...
regr.fit(np.column_stack((cfg_scale_train, ddim_steps_train, denoising_strength_train)), diff_similarity_train)

# Make predictions using the testing set
diff_similarity_pred = regr.predict(np.column_stack((cfg_scale_test, ddim_steps_test, denoising_strength_test)))
...
% mean_squared_error(diff_similarity_test, diff_similarity_pred))
# Explained variance score: 1 is perfect prediction
print('Variance score: %.2f' % r2_score(diff_similarity_test, diff_similarity_pred))

# Plot outputs in a 3d plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
#ax.scatter(cfg_scale_test, ddim_steps_test, denoising_strength_test, c=diff_similarity_test)
ax.scatter(cfg_scale_test, ddim_steps_test, denoising_strength_test, c=diff_similarity_pred)
...
m.set_array(diff_similarity_test)
#label the colorbar
plt.colorbar(m, label='diff_similarity_difference')
...
stats = {}
stats["name"] = "diff_similarity"
stats["correlations"] = {}
stats["correlations"]["cfg_scale"] = str(np.corrcoef(diff_similarity, cfg_scale)[0][1])
stats["correlations"]["ddim_steps"] = str(np.corrcoef(diff_similarity, ddim_steps)[0][1])
stats["correlations"]["diff_similarity"] = str(np.corrcoef(diff_similarity, denoising_strength)[0][1])
stats["coefficients"] = str(regr.coef_)
stats["mean_squared_error"] = str(mean_squared_error(diff_similarity_test, diff_similarity_pred))
stats["variance_score"] = str(r2_score(diff_similarity_test, diff_similarity_pred))
