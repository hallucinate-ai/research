...
for data in filtered_json_data_by_ddim_steps:
	mod_similiarity.append(float(data["scores"]["mod_similiarity"]))
...
print(np.corrcoef(mod_similiarity, cfg_scale))
print("dst_aesthetic_score vs ddim_steps")
print(np.corrcoef(mod_similiarity, ddim_steps))
print("dst_aesthetic_score vs denoising_strength")
print(np.corrcoef(mod_similiarity, denoising_strength))
plt.show()
...
length = int(round(len(mod_similiarity) / 2))
...
mod_similiarity_train = mod_similiarity[:length]
mod_similiarity_test = mod_similiarity[length:]

# Create linear regression object
regr = linear_model.LinearRegression()

# Train the model using the training sets
regr.fit(np.column_stack((cfg_scale_train, ddim_steps_train, denoising_strength_train)), mod_similiarity_train)

# Make predictions using the testing set
mod_similiarity_pred = regr.predict(np.column_stack((cfg_scale_test, ddim_steps_test, denoising_strength_test)))
...
% mean_squared_error(mod_similiarity_test, mod_similiarity_pred))
# Explained variance score: 1 is perfect prediction
print('Variance score: %.2f' % r2_score(mod_similiarity_test, mod_similiarity_pred))

# Plot outputs in a 3d plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
#ax.scatter(cfg_scale_test, ddim_steps_test, denoising_strength_test, c=diff_similarity_test)
ax.scatter(cfg_scale_test, ddim_steps_test, denoising_strength_test, c=mod_similiarity_pred)
...
m.set_array(mod_similiarity_test)
#label the colorbar
plt.colorbar(m, label='mod_similiarity')
...
stats = {}
stats["name"] = "mod_similiarity"
stats["correlations"] = {}
stats["correlations"]["cfg_scale"] = str(np.corrcoef(mod_similiarity, cfg_scale)[0][1])
stats["correlations"]["ddim_steps"] = str(np.corrcoef(mod_similiarity, ddim_steps)[0][1])
stats["correlations"]["diff_similarity"] = str(np.corrcoef(mod_similiarity, denoising_strength)[0][1])
stats["coefficients"] = str(regr.coef_)
stats["mean_squared_error"] = str(mean_squared_error(mod_similiarity_test, mod_similiarity_pred))
stats["variance_score"] = str(r2_score(mod_similiarity_test, mod_similiarity_pred))
