...
def process(dir):
	for filename in os.listdir(dir):
		if filename.endswith('.json'):
			...
			image_src = preprocess(Image.open(dir + "/" +filename.replace(".json", "_src.png"))).unsqueeze(0)
			image_dst = preprocess(Image.open(dir + "/" +filename.replace(".json", "_dst.png"))).unsqueeze(0)
