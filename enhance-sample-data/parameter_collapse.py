...
#arg1 = closeness to the original text
#arg2 = closeness to the original image

def parameter_collapse(arg1, arg2):

	def load_json(path):
		return json.load(open(path))

	json_data = load_json("samples_img2img.json")
	...
	#print("selected_dst_similarity: ", selected_dst_similarity) #similarity to the ptompt text
	selected_mod_similarity = (selected_norm2 * (max(mod_similarity) - min(mod_similarity))) + min(mod_similarity)
	#print("selected_mod_similarity: ", selected_mod_similarity) #similarity to the prompt image
	...
	return{"cfg_scale": selected_cfg_scale[0], "ddim_steps": int(round(selected_ddim_steps[0])), "denoising_strength": selected_denoising_strength[0], "predicted_dst_aesthetic_score": selected_dst_aesthetic_score[0], "selected_dst_similarity": selected_dst_similarity, "selected_mod_similarity": selected_mod_similarity}

if len(sys.argv) == 2:
	arg1 = sys.argv[1]
	arg2 = sys.argv[2]
	parameter_collapse(arg1, arg2)
print(parameter_collapse(1, 0))
#arg1 = closeness to the original text
#arg2 = closeness to the original image
