...
#filtered_json_data_by_prompt_length = [x for x in filtered_json_data_by_sampler_name if int(x["scores"]["prompt_length"]) < 128]	
...
print("aesthetic_score_difference vs cfg_scale")
print(np.corrcoef(aesthetic_score_difference, cfg_scale))
print("dst_aesthetic_score vs ddim_steps")
print(np.corrcoef(aesthetic_score_difference, ddim_steps))
print("dst_aesthetic_score vs denoising_strength")
print(np.corrcoef(aesthetic_score_difference, denoising_strength))
plt.show()
...
length = int(round(len(aesthetic_score_difference) / 2))
...
regr.fit(np.column_stack((cfg_scale_train, ddim_steps_train, denoising_strength_train)), aesthetic_score_difference_train)

# Make predictions using the testing set
aesthetic_score_difference_pred = regr.predict(np.column_stack((cfg_scale_test, ddim_steps_test, denoising_strength_test)))
...
% mean_squared_error(aesthetic_score_difference_test, aesthetic_score_difference_pred))
# Explained variance score: 1 is perfect prediction
print('Variance score: %.2f' % r2_score(aesthetic_score_difference_test, aesthetic_score_difference_pred))

# Plot outputs in a 3d plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
#ax.scatter(cfg_scale_test, ddim_steps_test, denoising_strength_test, c=dst_aesthetic_score_test)
ax.scatter(cfg_scale_test, ddim_steps_test, denoising_strength_test, c=aesthetic_score_difference_pred)
...
#label the colorbar
plt.colorbar(m, label='aesthetic_score_difference')
...
stats = {}
stats["name"] = "aesthetic_score_difference"
stats["correlations"] = {}
stats["correlations"]["cfg_scale"] = str(np.corrcoef(aesthetic_score_difference, cfg_scale)[0][1])
stats["correlations"]["ddim_steps"] = str(np.corrcoef(aesthetic_score_difference, ddim_steps)[0][1])
stats["correlations"]["diff_similarity"] = str(np.corrcoef(aesthetic_score_difference, denoising_strength)[0][1])
stats["coefficients"] = str(regr.coef_)
stats["mean_squared_error"] = str(mean_squared_error(aesthetic_score_difference_test, aesthetic_score_difference_pred))
stats["variance_score"] = str(r2_score(aesthetic_score_difference_test, aesthetic_score_difference_pred))
